...
def transcription_loss(y_true: tf.Tensor, y_pred: tf.Tensor, label_smoothing: float) -> tf.Tensor:
    ...
    """
    bce = tf.keras.losses.binary_crossentropy(y_true, y_pred, label_smoothing=label_smoothing, reduction=tf.keras.losses.Reduction.SUM)
    return bce


def weighted_transcription_loss(
    y_true: tf.Tensor, y_pred: tf.Tensor, label_smoothing: float, positive_weight: float = 10.0
) -> tf.Tensor:
    ...
    # Apply class weights
    weights = tf.where(tf.equal(y_true, 1), positive_weight, 1.0)

    # Weight the losses 
    weighted_bce = weights * bce
    
    # Sum over the last dimension (num_classes)
...
def loss(label_smoothing: float = 0.2, weighted: bool = False, positive_weight: float = 0.5) -> Dict[str, Any]:
    ...
    loss_contour = lambda x, y: weighted_transcription_loss(x, y, label_smoothing=label_smoothing, positive_weight=1.0)
    loss_note = lambda x, y: weighted_transcription_loss(x, y, label_smoothing=label_smoothing, positive_weight=1.0)
    loss_onset = lambda x, y: weighted_transcription_loss(x, y, label_smoothing=label_smoothing, positive_weight=10.0)
...
def loss_no_contour(label_smoothing: float = 0.2, weighted: bool = False, positive_weight: float = 0.5) -> Dict[str, Any]:
    """Creates a keras-compatible dictionary of loss functions to calculate
    the loss for the contour, note and onset posteriorgrams.
    ...
    loss_note = lambda x, y: transcription_loss(x, y, label_smoothing=label_smoothing)
    loss_onset = lambda x, y: weighted_transcription_loss(x, y, label_smoothing=label_smoothing, positive_weight=10.0)
...
def get_vqt(inputs: tf.Tensor, n_harmonics: int, use_batchnorm: bool) -> tf.Tensor:
    n_semitones = np.min([
        int(np.ceil(12.0 * np.log2(n_harmonics)) + ANNOTATIONS_N_SEMITONES),
        MAX_N_SEMITONES,
    ])
...
def get_spectrogram(inputs: tf.Tensor, use_batchnorm: bool) -> tf.Tensor:
    """
    Calculate the spectrogram of the input audio.
    ...
    Args:
        inputs: The audio input.
        use_batchnorm: If True, applies batch normalization after computing the spectrogram

    Returns:
        The log-normalized spectrogram of the input audio.
    """
    x = nn.FlattenAudioCh()(inputs)
    x = signal.Spectrogram(
        fft_length=FFT_HOP*8,
        hop_length=FFT_HOP,
...
def model(
    ...
    Args:
        ...
        initial_gamma: Initial value for the trainable gamma parameter.
    """
    # input representation
    inputs = tf.keras.Input(shape=(AUDIO_N_SAMPLES, 1))  # (batch, time, ch)
    x = get_vqt(inputs, n_harmonics, True)
    ...
    print(x_contours_reduced.shape)
...
def model_v2(
    n_harmonics: int = 8,
    n_filters_contour: int = 24,
    n_filters_onsets: int = 48,
    n_filters_notes: int = 48,
) -> tf.keras.Model:
    """Basic Pitch's model implementation.
    ...
    x_contours_out = nn.FlattenFreqCh(name="contour")(x_contours)

    # onset layers
    x_onset = tf.expand_dims(x_contours_out, -1)
    x_onset = tfkl.Conv2D(
        n_filters_onsets,
        (7, 7),
        padding="same",
    ...
    x_onset_out = nn.FlattenFreqCh(name="onset")(x_onset)

    # note layers
    x_notes = tf.expand_dims(x_contours_out, -1)
    x_notes = tfkl.Conv2D(
        n_filters_notes,
        (7, 7),
        padding="same",
    ...
    x_notes = tfkl.BatchNormalization()(x_notes)
    x_notes = tfkl.ReLU()(x_notes)
    x_notes = tfkl.Concatenate(axis=3, name="concat")([x_onset, x_notes])
    ...
    x_notes_out = nn.FlattenFreqCh(name="note")(x_notes)

    outputs = { "onset": x_onset_out, "contour": x_contours_out, "note": x_notes_out }
